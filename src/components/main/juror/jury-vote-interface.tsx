...
export function JuryVoteInterface({ 
    ...
    return (
      <div className="text-center space-y-3 p-4 bg-purple-50 dark:bg-purple-950/20 rounded-lg">
        ...
        <div className="text-sm text-purple-700 dark:text-purple-300 space-y-1">
          <p>Choice of Proposer: <span className="font-bold">{getOutcomeText(proposedOutcome)}</span> ({Number(votesForProposer)} votes)</p>
          <p>Choice of Disputor: <span className="font-bold">{getOutcomeText(disputedOutcome)}</span> ({Number(votesForDisputer)} votes)</p>
        </div>
  ...
  return (
    <div className="text-center space-y-3 p-4 bg-purple-50 dark:bg-purple-950/20 rounded-lg">
      ...
      <div className="text-sm text-purple-700 dark:text-purple-300 space-y-1">
        <p>Choice of Proposer: <span className="font-bold">{getOutcomeText(proposedOutcome)}</span> ({Number(votesForProposer)} votes)</p>
        <p>Choice of Disputer: <span className="font-bold">{getOutcomeText(disputedOutcome)}</span> ({Number(votesForDisputer)} votes)</p>
      </div>
      <p className="text-xs text-purple-600 dark:text-purple-400">
        Time remaining: {timeRemaining()}
      </p>
